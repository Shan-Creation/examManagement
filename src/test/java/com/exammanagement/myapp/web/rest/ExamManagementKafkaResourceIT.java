...
import com.exammanagement.myapp.config.KafkaSseConsumer;
import com.exammanagement.myapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class ExamManagementKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/exam-management-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/exam-management-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/exam-management-kafka/unregister"));
                return;
            }
        }
